...
const product = async function (req, res) {
    try {
        ...
        for (let i = 0; i < dataArr.length; i++) {
            ...
            if (!flag) return res.status(400).send({ status: false, message: `${dataArr[i]} field is mandatory ` });
            if (entries[j][k] == "title") {
                if (entries[j][1].trim() == "") return res.status(400).send({ status: false, message: `${entries[j][k]} field cannot be empty ` });
                if (data.title == "")
            ...
            if (entries[j][k] == "price") {
                if (entries[j][1].trim() == "") return res.status(400).send({ status: false, message: `${entries[j][k]} field cannot be empty ` });
                if (!Number(entries[j][1])) return res.status(400).send({ status: false, message: `${entries[j][k]} can only be a number and can not be 0 ` });
            }
            if (entries[j][k] == "currencyId") {
                if (entries[j][1].trim() == "") return res.status(400).send({ status: false, message: `${entries[j][k]} field cannot be empty ` });
                if (entries[j][1] != "INR") return res.status(400).send({ status: false, message: `${entries[j][k]} must be 'INR' ` });
            }
            if (entries[j][k] == "currencyFormat") {
                if (entries[j][1].trim() == "") return res.status(400).send({ status: false, message: `${entries[j][k]} field cannot be empty ` });
                if (entries[j][1] != "₹") return res.status(400).send({ status: false, message: `${entries[j][k]} must be '₹' ` });
            }
        }
        ...
        if (data.style || data.style == "") {
            if (data.style.trim() == "") return res.status(400).send({ status: false, message: "Enter value for style field" }); //delete data.style
        }

        if (data.installments || data.installments == "") {
            data.installments = data.installments.trim()
            if (data.installments == "") return res.status(400).send({ status: false, message: "Installment field can not be empty" });
            if (!Number(data.installments)) return res.status(400).send({ status: false, message: "Installment field can only contain Number" });
        }
        ...
        sizes.forEach(a => {
            if (availableSizesEnum.includes(a))
                arr.push(a)
        })
